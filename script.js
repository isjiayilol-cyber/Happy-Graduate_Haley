...
function updateCoverPhoto() {
  ...
  if (!img || !soloPhotos.length) return;
  img.src = soloPhotos[coverIndex % soloPhotos.length];
}
function nextCoverPhoto() { coverIndex = (coverIndex + 1) % soloPhotos.length; updateCoverPhoto(); }

function updateGroupSlide() {
  const img = document.getElementById('groupSlide');
  if (!img || !groupPhotos.length) return;
  img.src = groupPhotos[groupIndex];
  document.getElementById('groupCaption').innerText = groupCaptions[groupIndex % groupCaptions.length] + ` (${groupIndex + 1}/${groupPhotos.length}) — tap photo for next`;
}
function nextGroup() { groupIndex = (groupIndex + 1) % groupPhotos.length; updateGroupSlide(); }
function prevGroup() { playClickSound(); groupIndex = (groupIndex - 1 + groupPhotos.length) % groupPhotos.length; updateGroupSlide(); }

function updateSoloSlide() {
  const img = document.getElementById('soloSlide');
  if (!img || !soloPhotos.length) return;
  img.src = soloPhotos[soloIndex];
  document.getElementById('soloCaption').innerText = soloCaptions[soloIndex % soloCaptions.length] + ` (${soloIndex + 1}/${soloPhotos.length}) — tap photo for next`;
}
function nextSolo() { soloIndex = (soloIndex + 1) % soloPhotos.length; updateSoloSlide(); }
function prevSolo() { playClickSound(); soloIndex = (soloIndex - 1 + soloPhotos.length) % soloPhotos.length; updateSoloSlide(); }
function showRandomSolo() {
  ...
  updateSoloSlide();
  showPopup(soloCaptions[soloIndex % soloCaptions.length] + ' 📸');
}

function randomHaleyJoke() { return jokes[Math.floor(Math.random() * jokes.length)]; }
function removeBird() {
  playClickSound();
  document.getElementById('bird').classList.add('gone');
  showPopup('The bird has been safely escorted away. Grandma Haley is protected.');
}
